...
public class ZAlgorithm {
    ...
    public static void search(String pattern,String text) {
        ...
        int[] zArray = new int[sb.length()];
        calculateZ(zArray,sb.toString());
    ...
    private static void calculateZ(int[] zArray,String s) {

        //Z box left & right ends
        int left =0,right=0;
        for (int i=1;i<s.length();i++) {
            ...
            if(i>right) {
                ...
                // and L = 2
                /*
                    Z[k] would be longest substring starting at k which is also prefix of the string.
                    Now R-i + 1 gives the length of remaining characters in the Z box.
                    Since we copy the value of Z[k] to optimize the calculation of Z values we need to check whether these values are
                    inside the boundary of the Z box. Hence this condition.
                 */
...
then total time taken is O(m + n) with linear space complexity. Now we can see that both time and space complexity is same as
KMP algorithm but this algorithm is Simpler to understand.
...
The idea is to concatenate pattern and text, and create a string “P$T” where P is pattern, $ is a special character should not
be present in pattern and text, and T is text. Build the Z array for concatenated string. In Z array, if Z value at any point is
equal to pattern length, then pattern is present at that point.
...
The algorithm runs in linear time because we never compare character less than R and with matching we increase R by
one so there are at most T comparisons. In mismatch case, mismatch happen only once for each i (because of which R stops),
that’s another at most T comparison making overall linear complexity.
